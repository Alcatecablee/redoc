 ...
 */
export async function generateAICaption(context, maxLength = 120) {
 ...
 */
function generateFallbackCaption(context, maxLength) {
 ...
 */
export async function generateCaptionsForImages(images, concurrency = 3) {
    ...
    for (let i = 0; i < images.length; i += concurrency) {
        ...
        await Promise.allSettled(batch.map(item => processImage(item).catch(error => {
 ...
 */
export async function enhanceImagesWithCaptions(images, contextMap = new Map()) {
    // Prepare batch
 ...
 */
export function extractNearbyContext(imageElement, maxLength = 300) {
    ...
    if (parent) {
        ...
        const before = siblings.slice(Math.max(0, imageIndex - 2), imageIndex)
            .map((i, el) => parent.find(el).text().trim())
            .get()
            .join(' ');
