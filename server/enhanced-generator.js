...
import { shouldSkipImage, fetchImageMetadata, deduplicateImages, generateShortCaption, sanitizeImageUrl, imageCache } from './image-utils';
import { composeImagesIntoDocumentation } from './image-composition';
...
async function fetchSitemaps(baseUrl, extraHosts = []) {
...
export async function discoverSiteStructure(baseUrl) {
    try {
...
export async function extractMultiPageContent(urls) {
    const extracted = [];
        ...
        try {
            ...
            $('img').each((i, el) => {
                ...
                try {
                    ...
                    imagePromises.push(fetchImageMetadata(sanitizedUrl, alt, url).then(metadata => {
            ...
            const validImages = imagesMetadata
                .filter((result) => result.status === 'fulfilled' && result.value.isValid === true)
...
export async function performExternalResearch(productName, baseUrl, crawledPageCount = 0, youtubeApiAccess = false, youtubeTranscripts = false) {
    console.log(`🔬 Performing comprehensive external research for: ${productName}`);
    try {
        const research = await searchService.performComprehensiveResearch(productName, baseUrl, 'medium', // Will be auto-estimated based on crawledPageCount
        crawledPageCount, youtubeApiAccess, youtubeTranscripts);
...
export async function generateEnhancedDocumentation(url, userId, sessionId, userPlan = 'free') {
    ...
    const candidateUrls = Array.from(new Set([
        ...
        ...siteStructure.allInternalLinks.filter((u) => docLikePatterns.test(u))
    ]));
    // First pass
    ...
    const externalResearch = await performExternalResearch(siteStructure.productName, url, extractedContent.length, tierLimits.youtubeApiAccess, tierLimits.youtubeTranscripts);
    // Partial success notification via progress and pipeline monitor if sources missing
    const missing = [];
    if (!process.env.SERPAPI_KEY && !process.env.BRAVE_API_KEY)
        missing.push('Search APIs');
    if (externalResearch.github_issues.length === 0)
        missing.push('GitHub issues');
...
IMPORTANT: Add source attribution for all solutions - format as "Source: [Stack Overflow #123](url)" or "Source: [GitHub Issue #456](url)" or "Source: [Official Docs](url)". Return JSON with sections array.`
    ...
    const extractedStructure = await parseJSONWithRetry(aiProvider, stage1Response.content, 'Ensure the output is valid JSON matching the comprehensive structure extraction format');
...
Return JSON array of sections: [{id: string, title: string, priority: "high"|"medium"|"low", rationale: string}]`;
    ...
    if (userPlan !== 'free') {
        ...
        try {
            ...
            seoMetadata = await seoService.generateSEOMetadata(siteStructure.productName, url, extractedContent.map(c => c.content).join(' ').substring(0, 5000), {
            ...
            schemaMarkup = await schemaService.generateSchemaMarkup(siteStructure.productName, sections, externalResearch.youtube_videos || [], url);
...
Return valid JSON with source attribution embedded in content.`
    ...
    const writtenDocs = await parseJSONWithRetry(aiProvider, stage2Response.content, 'Ensure the output is valid JSON with proper comprehensive documentation structure');
    ...
    if (allImages.length > 0) {
        try {
            ...
            const documentSections = enhancedSections.map((section, index) => ({
                id: section.id || `section-${index}`,
                title: section.title || '',
                content: section.content?.map((block) => block.text || '').join(' ') || '',
                blocks: section.content || [],
                headingLevel: 2
            }));
            // Compose images into documentation
            const compositionResult = composeImagesIntoDocumentation(documentSections, enhancedImages);
            // Update sections with composed images
            enhancedSections = compositionResult.sections.map(section => ({
                id: section.id,
                title: section.title,
                icon: enhancedSections.find((s) => s.id === section.id)?.icon || 'BookOpen',
...
Return JSON: {metadata: {title, description, keywords}, searchability: {primary_tags, search_keywords}, validation: {status}}.`
    ...
    const finalMetadata = await parseJSONWithRetry(aiProvider, stage3Response.content, 'Ensure the output is valid JSON with metadata and searchability fields');
...
function extractThemeFromContent(pages) {
    const allColors = [];
    const allFonts = [];
    pages.forEach(page => {
