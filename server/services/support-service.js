...
export class SupportService {
    ...
    async createTicket(userId, subject, description, priority = 'normal') {
    ...
    async getTicket(ticketId, userId) {
        ...
        const ticket = await database.query.supportTickets.findFirst({
            where: and(eq(supportTickets.id, ticketId), eq(supportTickets.user_id, userId)),
        });
        if (!ticket) {
            return null;
        }
    ...
    async addMessage(ticketId, userId, message, isInternal = false) {
        const database = this.ensureDb();
