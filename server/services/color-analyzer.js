import { parseColor, rgbToHex, getContrastRatio, meetsWCAG_AA } from '@shared/themes';
export class ColorAnalyzer {
    /**
     * Perform K-means clustering on colors to find dominant groups
     */
    kMeansClustering(colors, k = 5, maxIterations = 100) {
        ...
        while (iterations < maxIterations) {
            ...
            const newClusters = centroids.map(c => ({ centroid: c, colors: [] }));
            ...
            for (let i = 0; i < k; i++) {
                if (newClusters[i].colors.length === 0)
                    continue;
     ...
     */
    adjustColorForContrast(color, background, targetRatio, lighten = false) {
        const rgb = parseColor(color);
        if (!rgb)
            return color;
     ...
     */
    centroidsEqual(c1, c2, epsilon = 0.1) {
        return (Math.abs(c1.l - c2.l) < epsilon &&
