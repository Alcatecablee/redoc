...
router.get('/api/organizations', verifySupabaseAuth, async (req, res) => {
    try {
        const user = req.user;
        if (!user || !user.email)
...
router.get('/api/organizations/:id/members', async (req, res) => {
    try {
        const user = req.user;
        if (!user || !user.email)
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/api/organizations/:id/members', async (req, res) => {
    try {
        const user = req.user;
        if (!user || !user.email)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!orgId || !email)
            return res.status(400).json({ error: 'Missing org id or email' });
        if (!db)
            return res.status(500).json({ error: 'Database not initialized' });
        ...
        if (!callerMembership || !['owner', 'admin'].includes(callerMembership.role))
            return res.status(403).json({ error: 'Forbidden: only owners or admins can add members' });
...
// Update member role
router.put('/api/organizations/:id/members/:memberId', async (req, res) => {
    try {
        const user = req.user;
        if (!user || !user.email)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (!callerMembership || callerMembership.role !== 'owner')
            return res.status(403).json({ error: 'Only owners may change roles' });
...
// Remove member
router.delete('/api/organizations/:id/members/:memberId', async (req, res) => {
    try {
        const user = req.user;
        if (!user || !user.email)
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const callerMembership = (await db.select().from(organizationMembers).where(eq(organizationMembers.organization_id, orgId)).where(eq(organizationMembers.user_id, currentUser.id)))[0];
        if (!callerMembership || !['owner', 'admin'].includes(callerMembership.role))
            return res.status(403).json({ error: 'Only owners or admins may remove members' });
