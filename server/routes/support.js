...
router.post('/api/support/tickets', verifySupabaseAuth, async (req, res) => {
    try {
        ...
        const ticket = await supportService.createTicket(parseInt(userId), subject, description, priority || 'normal');
        res.json({
            success: true,
            ticket,
        });
...
router.post('/api/support/tickets/:id/messages', verifySupabaseAuth, async (req, res) => {
    try {
        ...
        const newMessage = await supportService.addMessage(ticketId, parseInt(userId), message, false);
        res.json({
            success: true,
            message: newMessage,
        });
