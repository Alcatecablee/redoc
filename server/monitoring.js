...
export class MonitoringService {
    static metrics = [];
    static maxMetrics = 1000; // Keep last 1000 metrics
    /**
     * Record a performance metric
     */
    static recordMetric(operation, duration, success, error, metadata) {
        const metric = {
            operation,
            duration,
            success,
     ...
     * Clear old metrics
     */
    static clearOldMetrics(olderThanHours = 24) {
    ...
    static getHealthStatus() {
        const recentMetrics = this.metrics.filter(m => m.timestamp > new Date(Date.now() - 60 * 60 * 1000) // Last hour
        );
