...
export class VideoContentAnalyzer {
     ...
     */
    async generateVideoSummary(video) {
        try {
            if (!this.aiProvider) {
...
Example: ["React Hooks", "State Management", "useEffect", "Custom Hooks", "Performance Optimization"]`;
     ...
     */
    prepareContentForAnalysis(video) {
        const parts = [
            `Title: ${video.title}`,
            `Channel: ${video.channelTitle}`,
            `Duration: ${video.duration}`,
     ...
     */
    async generateAIAnalysis(content, video) {
        if (!this.aiProvider) {
...
Focus on technical content and educational value.`;
     ...
     */
    async extractTimestampsFromContent(video) {
        const timestamps = [];
    ...
    extractTopicsFromText(text) {
        ...
        const foundTopics = commonTechTerms.filter(term => text.toLowerCase().includes(term.toLowerCase()));
