...
import { canGenerateDocumentation } from './tier-config';
...
async function parseJSONWithRetry(apiKey, content, retryPrompt, maxRetries = 2) {
    let lastError = null;
...
router.post("/api/generate-docs", verifySupabaseAuth, async (req, res) => {
    try {
        ...
        console.log('/api/generate-docs called', {
            headers: Object.keys(req.headers).reduce((acc, key) => ({ ...acc, [key]: req.headers[key] }), {}),
            body: req.body,
            ip: req.ip,
        });
...
Available images: ${images.slice(0, 10).join(', ')}`
            ...
            const extractedStructure = await parseJSONWithRetry(OPENAI_API_KEY, stage1Data.choices?.[0]?.message?.content || '{}', 'Ensure the output is valid JSON matching the structure extraction format');
...
Use proper formatting, include relevant images, and make it professional and comprehensive. Return ONLY valid JSON.`
            ...
            const writtenDocs = await parseJSONWithRetry(OPENAI_API_KEY, stage2Data.choices?.[0]?.message?.content || '{}', 'Ensure the output is valid JSON with proper documentation structure');
...
Source URL: ${url}`
            ...
            const finalMetadata = await parseJSONWithRetry(OPENAI_API_KEY, stage3Data.choices?.[0]?.message?.content || '{}', 'Ensure the output is valid JSON with metadata and searchability fields');
...
Return ONLY valid JSON.`
            ...
            if (stage4Response.ok) {
                ...
                const validationData = await parseJSONWithRetry(OPENAI_API_KEY, stage4Data.choices?.[0]?.message?.content || '{}', 'Ensure the output is valid JSON with validation results and refined sections');
        ...
        parsedContent.sections?.forEach((section) => {
            ...
            try {
                if (Array.isArray(_citationsRoot)) {
                    ...
                    if (section.id && _citationsRoot[section.id])
                        sectionCites.push(..._citationsRoot[section.id]);
                    if (section.title && _citationsRoot[section.title])
                        sectionCites.push(..._citationsRoot[section.title]);
                    if (_citationsRoot.sections && section.id && _citationsRoot.sections[section.id])
                        sectionCites.push(..._citationsRoot.sections[section.id]);
                }
                if (section.citations && Array.isArray(section.citations))
                    sectionCites.push(...section.citations);
            }
            catch (e) {
...
`;
        ...
        parsedContent.sections?.forEach((section) => {
            ...
            sectionCites.forEach((c) => {
                ...
                if (typeof c === 'string')
                    s = c;
                else if (c.title && c.url)
                    s = `<a href="${c.url}">${c.title}</a>`;
                else if (c.url)
                    s = `<a href="${c.url}">${c.url}</a>`;
                else
                    s = `<code>${JSON.stringify(c)}</code>`;
                if (!seen.has(s)) {
                    seen.add(s);
                    normalized.push(s);
                }
...
router.get("/api/export/pdf/:id", verifySupabaseAuth, async (req, res) => {
    try {
        ...
        const PDFDocument = (pdfModule && (pdfModule.default || pdfModule));
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ error: "Invalid ID" });
        }
                    ...
                    switch (block.type) {
                        ...
                        case 'image':
                            try {
                                ...
                                let imageOptions = { fit: [maxImageWidth, maxImageHeight], align: 'center' };
...
router.get("/api/export/docx/:id", verifySupabaseAuth, async (req, res) => {
    try {
        ...
        const { Document, Packer, Paragraph, TextRun, HeadingLevel, ImageRun } = docxModule;
        const id = parseInt(req.params.id);
        if (isNaN(id)) {
            return res.status(400).json({ error: "Invalid ID" });
        }
...
function generateSubdomain(url, title) {
    try {
...
router.post("/api/export/subdomain/:id", verifySupabaseAuth, async (req, res) => {
    try {
        ...
        const currentSubdomain = doc.subdomain;
        if (currentSubdomain) {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                updatedDoc = await storage.updateDocumentation(id, { subdomain });
...
router.get('/api/consulting/success', async (req, res) => {
    try {
        ...
        const orderDetails = await getOrder(token);
      ...
      `);
        ...
        if (captureResult.status === 'COMPLETED') {
            ...
            setImmediate(async () => {
                try {
                    const result = await generateDocumentationPipeline(metadata.url, captureResult.payerId || null, sessionId, 'enterprise' // Consulting orders get enterprise features
                    );
