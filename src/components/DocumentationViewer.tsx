      ...
      case "image":
        ...
        const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
          ...
          if (figure) {
            ...
            errorMsg.className = 'p-4 bg-destructive/10 border border-destructive/20 rounded-lg text-sm text-destructive';
            errorMsg.textContent = `Failed to load image: ${block.alt || 'Image unavailable'}`;
        ...
        return (
          <figure key={index} className="mb-6">
            <img 
              ...
              className="w-full max-w-3xl rounded-lg border shadow-md transition-opacity duration-300"
              loading="lazy"
              decoding="async"
            ...
            {block.source && (
              <figcaption className="mt-1 text-xs text-muted-foreground/70 text-center">
                Source: {block.source}
              </figcaption>
            )}
          </figure>
